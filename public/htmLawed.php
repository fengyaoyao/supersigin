...
*/
function fliterHtmlSpecialChars($string){
	$string = preg_replace('/&amp;((#(\d{3,5}|x[a-fA-F0-9]{4})|[a-zA-Z][a-z0-9]{2,5});)/', '&\\1',
			str_replace(array('&', '"', '<', '>'), array('&amp;', '&quot;', '&lt;', '&gt;'), $string));
	return $string;
}
function getAddslashesForInput($array,$ignore=array()){
	if(!empty($array)){
		foreach($array as $k => $v){
			if(is_string($v)){
				...
				if($k == 'statistics_code'){
					...
					$array[$k] = getAddslashesForInput($v);
				}
			}
		}
		return $array;
