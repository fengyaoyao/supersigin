...
class PostedController extends HomeBaseController
{
    public function index()
    {
        ...
        session('user', $user);
        //
        $result = Db::name("user_posted")->where("uid=$uid and status = 1 and is_open_super_sign != 1  and endtime>" . time())->where($map)->order("id desc")->paginate(10);
        $zuoc = $this->zuoc($uid);
        // var_dump($zuoc);die();
    ...
    public function supindex(){
        ...
        $auth_info = Db::name('user_auth_info')->where('user_id', $uid)->find();
        if ($auth_info) {
            if ($auth_info['status'] == 1) {
                $status = '已认证';
            } else {
        ...
        $user = Db::name("user")->where("id=$uid ")->find();
        session('user', $user);//
        $result = Db::name("user_posted")->where("uid=$uid and status = 1 and is_open_super_sign = 1  and endtime>" . time())->order("id desc")->paginate(10);
    ...
    public function pay_super(){
        ...
        if($pay_type==1){
            ...
            //echo $r;die();
            if($r){
                $url = 'http://'.$_SERVER['HTTP_HOST'].'/lib/alipay/wappay/pay.php';
