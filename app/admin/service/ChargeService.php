...
class ChargeService
{
    ...
    public static function charge(){
        ...
        $log = $model->order('addtime desc')
                ...
                ->each(function($items){
                    $u_info = Db::name('user')->where('id',$items['uid'])->field('user_nickname,mobile,sup_down_public')->find();
                   
                    $items['u_name']='该用户已被删除';
                    $items['sup_down_public'] = $u_info['sup_down_public'];
                    if($u_info){
    ...
    public static function chargexizai(){
        $params = request()->param();
        $model = Db::name('super_download_log');
        $_params=[];
        /*if(isset($params['uid']) && $params['uid']){
            $_params['uid'] = $params['uid'];
            $model = $model->where('uid',$params['uid']);
        ...
        $log = $model->order('addtime desc')
            ->paginate(10,false,['query'=>$_params])
            ->each(function($items){
                $u_info = Db::name('user')->where('id',$items['uid'])->field('id,user_nickname,mobile,sup_down_public')->find();
                $items['u_name']=$u_info['id'].'('.$u_info['mobile'].')';

                $ppp = Db::name('user_posted')
                    ->where('id',$items['app_id'])
                    ->find();
