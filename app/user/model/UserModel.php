...
class UserModel extends Model
{
    ...
    public function registerEmail($user,$is_session=true)
    {
        $userQuery = Db::name("user");
        $result    = $userQuery->where('user_email', $user['user_email'])->find();
    ...
    {
        $result = Db::name("user")->where('user_login', $user['username'])->find();
    ...
    /*七牛配置*/
    public function cattleData($user)
    {
        $userId           = cmf_get_current_user_id();
        $data['accessKey'] = trim($user['accessKey']);
        $data['secretKey'] = trim($user['secretKey']);
        $data['bucket'] = trim($user['bucket']);
        $data['domain'] = trim($user['domain']);
        $userQuery        = Db::name("user");
        if ($userQuery->where('id', $userId)->update($data)) {
            $userInfo = $userQuery->where('id', $userId)->find();
            cmf_update_current_user($userInfo);
